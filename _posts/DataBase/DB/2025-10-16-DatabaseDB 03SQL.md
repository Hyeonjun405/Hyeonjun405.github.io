...
### 4. 함수
#### 1. 단일행 함수(단일 행 기준으로 뭔가 처리)

| 구분    | 주요 함수                                                  | 설명                      |
| ----- | ------------------------------------------------------ | ----------------------- |
| 숫자 함수 | `ABS`, `ROUND`, `CEIL`, `FLOOR`, `MOD`                 | 절댓값, 반올림, 올림, 내림, 나머지 등 |
| 문자 함수 | `UPPER`, `LOWER`, `SUBSTR`, `LENGTH`, `CONCAT`, `TRIM` | 문자열 변환, 자르기, 길이 구하기 등   |
| 날짜 함수 | `SYSDATE`, `ADD_MONTHS`, `MONTHS_BETWEEN`, `NEXT_DAY`  | 날짜 계산 및 비교              |
| 변환 함수 | `TO_CHAR`, `TO_DATE`, `TO_NUMBER`                      | 데이터 타입 간 변환             |
| 기타    | `NVL`, `NVL2`, `DECODE`, `CASE`                        | NULL 처리 및 조건 분기         |

#### 2. 그룹함수 (여러행을 이용해서 집계 도출)
