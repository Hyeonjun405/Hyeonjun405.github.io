...
### 1. 과금정산플랫폼 노트(2025.11.19.)
 ...
 - 최종 대안
   ...
   if( session에 userInfo 유무){
    ...
    // UserInfo정보의 ID이용해서 접근 가능 scrn_cd 확인 
    // 기타 메뉴들 요청시에는 scrn_cd값이 없으니 메인으로 넘김
    // 그 외 권한이 없는 페이지를 요청하는 경우도 main 페이지 리턴 
   }else{
    // userInfo가 없는건 로그인을 한번도 성공하지 못한 상태
    // 로그인을 위한 최초 접근과 RestAPI요청 및 의도하지 않는 접근들은 여기에 걸리게 됨
    // 또한 한번 접근후에 화면 변경등으로 VIEW를 재요청하거나 비동기로 데이터 조회하는 것은 선행 IF에서 잡혀서 안찍힘.
    // 내부 인터셉터에서 어떤 화면에서 어떤 작업을 하고 있는지만 캐치하면 됨.
    // 따라서 이 타이밍에서 기본적인 로그 기록 (★)
    // RestAPI는 기존에 header에 Key값을 설정해서 일치여부 검증하므로 Header에 해당 Key가 있는지 확인
    // => RestAPI는 성공 결과를 반환하면 서버에서 확인이 가능하지만, 그 외에 AP로 직접 와버리면 파악이 안되니 바로 전단계에서 기록 
    // 나머지 로그인을 위한 최초 접근과 의도하지 않은 접근은 Login.jsp로 반환
   }
   ``` 
